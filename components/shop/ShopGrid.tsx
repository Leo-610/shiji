...
export function ShopGrid({
  ...
  return (
    <div className="space-y-6">
      ...
      {(["avatar_frame", "title_badge"] as const).map((sectionType) => {
        const sectionItems = catalog.filter((item) => item.type === sectionType);
        ...
        return (
          <section key={sectionType} className="space-y-4">
            <h2 className="text-sm font-orbitron tracking-widest text-theme-accent opacity-80 uppercase">
              {sectionType === "avatar_frame" ? "头像框" : "称号"}
            </h2>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {!item.owned ? (
                      <Button
                        size="sm"
                        disabled={
                          loadingSlug === item.slug || points < item.price
                        }
